...
import ch.felberto.service.AssociationQueryService;
...
/**
 * REST controller for managing {@link Association}.
 */
@RestController
@RequestMapping("/api")
public class AssociationResource {
    ...
    public ResponseEntity<Association> updateAssociation(
        ...
        log.info("REST request to update Association : {}, {}", id, association);
        if (association.getId() == null) {
     ...
     * @throws URISyntaxException if the Location URI syntax is incorrect.
     */
    @PatchMapping(value = "/associations/{id}", consumes = "application/merge-patch+json")
    public ResponseEntity<Association> partialUpdateAssociation(
        @PathVariable(value = "id", required = false) final Long id,
        @NotNull @RequestBody Association association
    ) throws URISyntaxException {
        log.info("REST request to partial update Association partially : {}, {}", id, association);
        if (association.getId() == null) {
            throw new BadRequestAlertException("Invalid id", ENTITY_NAME, "idnull");
        }
        if (!Objects.equals(id, association.getId())) {
    ...
    public ResponseEntity<List<Association>> getAllAssociations(AssociationCriteria criteria, Pageable pageable) {
        log.debug("REST request to get Associations by criteria: {}", criteria);
        Page<Association> page = associationQueryService.findByCriteria(criteria, pageable);
