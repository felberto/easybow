...
export class RankingService {
  ...
  update(ranking: IRanking): Observable<EntityResponseType> {
    return this.http.put<IRanking>(`${this.resourceUrl}/${getRankingIdentifier(ranking) as number}`, ranking, {
      observe: 'response',
    });
  }

  partialUpdate(ranking: IRanking): Observable<EntityResponseType> {
    return this.http.patch<IRanking>(`${this.resourceUrl}/${getRankingIdentifier(ranking) as number}`, ranking, {
      observe: 'response',
    });
  }
  ...
  findByCompetition(competition: ICompetition): Observable<HttpResponse<Array<IRanking>>> {
    return this.http.get<Array<IRanking>>(`${this.resourceUrl}/competition/${getCompetitionIdentifier(competition) as number}`, {
      observe: 'response',
    });
  }
  ...
  deleteAllByCompetition(competition: ICompetition): Observable<HttpResponse<{}>> {
    return this.http.delete(`${this.resourceUrl}/competition/${getCompetitionIdentifier(competition) as number}`, { observe: 'response' });
  }
