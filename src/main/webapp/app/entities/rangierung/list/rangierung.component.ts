...
export class RangierungComponent implements OnInit {
  ...
  protected onSuccess(data: IRangierung[] | null, headers: HttpHeaders, page: number, navigate: boolean): void {
    this.totalItems = Number(headers.get('X-Total-Count'));
    this.page = page;
    if (navigate) {
      this.router.navigate(['/rangierung'], {
        queryParams: {
          page: this.page,
          size: this.itemsPerPage,
