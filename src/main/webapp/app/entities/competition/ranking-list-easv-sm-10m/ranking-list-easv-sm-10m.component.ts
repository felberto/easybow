...
@Component({
  selector: 'jhi-ranking-list-easv-sm-10m',
  templateUrl: './ranking-list-easv-sm-10m.component.html',
  styleUrls: ['./ranking-list-easv-sm-10m.component.scss'],
})
export class RankingListEasvSm10mComponent implements OnInit {
  ...
  ngOnInit(): void {
    this.activatedRoute.data.subscribe(({ competition }) => {
      ...
      this.rankingListService.getRankingList(competition, 0).subscribe(res => {
      ...
      if (this.rankingList.type === 101) {
        ...
        this.rankingListService.printRankingList(this.rankingList).subscribe(data => {
          ...
          if (
            ...
            title = this.rankingList.competition.name.toString() + ' ' + this.rankingList.competition.year.toString() + '.pdf';
          }
          link.download = title;
          link.click();
