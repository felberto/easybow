...
@Component({
  selector: 'jhi-overview-easv-sm-10m',
  templateUrl: './competition-overview-easv-sm-10m.component.html',
  styleUrls: ['./competition-overview-easv-sm-10m.component.scss'],
})
export class CompetitionOverviewEasvSm10mComponent implements OnInit {
  competition!: ICompetition;
  filterTerm: any = '';
  filteredItems: Array<IResults> | null = [];
  ...
  filterItems(): void {
    if (this.filterTerm === null) {
      ...
      this.filteredItems = this.results!.filter(
        item =>
          parseInt(this.filterTerm, 10) >= parseInt(item.athleteNumber!, 10) &&
          parseInt(this.filterTerm, 10) <= parseInt(item.athleteNumber!, 10) + 7
      );
    }
  }
